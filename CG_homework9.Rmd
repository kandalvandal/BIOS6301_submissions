...
boot_interval <- function(sample_size) {
  for (k in (1:1000)) { # loop to do the bootstrapping, I find his simpler than doing replicate since I can add means to a vector for later
    treatment <- rbinom(sample_size, 1, 0.5)
    outcome <- rnorm(sample_size, mean=60, sd=20)
      for (i in 1:sample_size) {
        if (treatment[i]==1){
...
boot_data <- rbind(boot_interval(250), boot_interval(500), boot_interval(750), boot_interval(1000), boot_interval(1250), boot_interval(1500), boot_interval(1750), boot_interval(2000), boot_interval(2250), boot_interval(2500))
