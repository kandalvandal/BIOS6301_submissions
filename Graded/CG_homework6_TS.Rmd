...
**39/40, great work! See my comments. Tianyi**
...
# TS
haart = read.csv('~/Documents/VUMC 2022 Fall Semester/BIOS6301/haart.csv', stringsAsFactors=FALSE)
```
...
for (i in 1:(nrow(haart))){
  if (!is.na(haart[,'date.death'][i])){ # there's a death date
    if (difftime(haart['date.death'][i,],haart['init.date'][i,], units='days') <= 365.25){
      ...
      death_indic[i] <- 0
  } # TS: I feel this should be else instead of else if?
}
...
##TS: It might be better to avoid for loop when create a variable, this could take some time when you have a large sample size.
death_indic = ifelse(!is.na(haart[,'date.death'])&(difftime(haart[,'date.death'],haart[,'init.date'], units='days') <= 365.25),1,0)
sum(death_indic, na.rm=T) 
```
...
# TS: The value of difference could be carried over if you didn't cover all possible scenarios regarding haart['date.death'], haart['last.visit']
for (i in 1:(nrow(haart))){
  if (!is.na(haart['date.death'][i,] > haart['last.visit'][i,])){ # both values present, death date is later
    # This condition would be TRUE as long as both date.death and last.visit are not missing
...
# The quantile for 25% was not exact. -0.5

# Alternative coding
haart$last.visit = as.POSIXct(haart$last.visit,format = '%m/%d/%y')
haart$followup = difftime(pmin(haart$last.visit,haart$date.death,na.rm = T), haart$init.date,units = 'days')

# censor at one year
haart[haart[,'followup'] > 365, 'followup'] <- 365
...
# TS: The goal is to find which individual drug was prescribed over 100 times
colnames(reg_drugs)[colSums(reg_drugs)>100]
# "3TC" "AZT" "EFV" "NVP" "D4T" are prescribed over 100 times -0.5
...
```{r echo=TRUE, error=TRUE}
# TS: It would be more straightforwad to answer this question if we code all above process into a function.
...
# first five records
# TS: Try head() and tail() functions
haart_final[1:5,]
