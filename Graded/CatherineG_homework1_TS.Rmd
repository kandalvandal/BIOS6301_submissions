...
and `exercise`.  In addition `exercise` is an ordered factor. `age` and `los` (length of stay) are continuous variables.  

* ordered factor - arranges the defined levels in increasing order

```{r}
gender <- c('M','M','F','M','F','F','M','F','M')
age <- c(34, 64, 38, 63, 40, 73, 27, 51, 47)
smoker <- c('no','yes','no','no','yes','no','no','no','yes')
exercise <- factor(c('moderate','frequent','some','some','moderate','none','none','moderate','moderate'), levels=c('none','some','moderate','frequent'), ordered=TRUE)
...
The `lm` function (linear regression) will take two arguments, a formula and a data set. The formula is split into two parts, where the vector to the left of
...
When choosing terms for a model, use prior research, _don't_ just
...
* In this case, `predict(mod)` and `fitted(mod)` give the same result, but they are not always interchangeable!
...
* To square the values, either do x^2 or x*x
* deviance() = calculates a goodness-of-fit statistic for the model, small=better fit
...
and `gender`.  The _residual degrees-of-freedom_ is the number of
...
`women`. (4 points)  
* which() = returns the position of dataframe elements meeting a condition (M or F)
* In this case it's used to index the positions of males/females, which are then extracted from the data frame
* Comma indicates I'm interested in the row number (I think)
...
sets. (3 points)  
* var() = returns distance from mean, reflects variability of sample
...
```
```
The second t-test (with var.equal=TRUE) matches the gender p-value from summary(mod)
```
