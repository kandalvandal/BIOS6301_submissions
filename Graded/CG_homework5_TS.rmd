...
**37/40 Good job! See comments below. -Tianyi**
...
**-2: the for loop should break when we find the root -TS**
...
#########################################
#TS###########
# Suggested input object: initial value (x), function, derivative 
# Logic: while abs(f(x))> tolerance and the number of iteration doesn't reach to the maximum -> loop; otherwise, loop breaks
newton_raphson <- function(x, f0=f, fp=d, tol=1e-9, n=1000) {
    i=0
    while(abs(f0(x)) > tol & i<=n) {
        x <- x - (f0(x)/fp(x))
        i=i+1
    }
    if(i == n) {
      ...
      return(NULL)
    } else print(sprintf('converges at %s', i))
    x
}
newton_raphson(x=10)
...
# Logic: while abs(f(x))> tolerance and the number of iteration doesn't reach to the maximum -> loop; otherwise, loop breaks
secant <- function(f, x0, x1, tol=1e-9, n=1000){
  guess <- x1 - f(x1)*(x1-x0)/(f(x1)-f(x0))
  iter_s <- 0
  while((abs(x0-x1)) > tol & iter_s<=n) {
    x0=x1
    x1=guess
    guess <- x1 - (f(x1)*(x1-x0)) / (f(x1)-f(x0))
    iter_s=iter_s+1
...
secant (f=f, x1=10, x0=9)
```

```{r}
# TS: See my revision below
# which is faster?

# Newton-Raphson, works here for some reason
start.time <- Sys.time()
newton_raphson(x=10, tol=1e-9, n=1000)
end.time <- Sys.time()
...
**20 points**
**-1:  The output function should print the sequence of rolls, not just the result. TS**
...
craps <- function(g) {
  ...
  for (i in 1:g) {
    ...
    # To show the game process:
    if (x1==7 || x1==11) {
      wins[i] <- 'win'
      print(sprintf("First roll: %s, win",x1))
    } else {
      print(sprintf("First roll: %s",x1))
      repeat {
      x2 <- sum(ceiling(6*runif(2)))
      print(sprintf("Next roll: %s",x2))
      if (x2==x1) {
        wins[i] <- 'win'
        print("win")
        break
...
craps <- function(verbose=TRUE) {
  # function to roll two die
  roll <- function() sum(ceiling(6*runif(2)))
  init <- roll()
  if(init %in% c(7,11)) {
    if(verbose) print(sprintf("First roll: %s, you win :)", init))
    return(TRUE)
  }
  if(verbose) print(sprintf("First roll: %s, roll again", init))
  repeat {
    x <- roll()
    if(x %in% c(init, 7, 11)) break
    if(verbose) {
      print(sprintf("You rolled %s, roll again", x))
    }
  }
  if(x == init) {
    if(verbose) print(sprintf("You rolled %s, you win :)", x))
    return(TRUE)
  }
  if(verbose) print(sprintf("You rolled %s, you lose :(", x))
  FALSE
}
set.seed(100)
craps()
...
funs_num_args[which.max(funs_num_args)] # 22 arguments
